...
class RAFDB_Trainer(Trainer):
  def __init__(self, model, train_loader, val_loader, test_loader,test_loader_ttau, configs, dir_weight, wb = True):
...
# Move the model to the device
    ...
    self.checkpoint_path = os.path.join(self.checkpoint_dir,"ResnetDuck_Cbam_cuaTuan_"+dir_weight)
    # load dataset
...
#       amsgrad = True,
    ...
    print(f"learning rate value = {self.learning_rate}")
  ...
  def Train_model(self):
    ...
    confusion_matrix, confusion_matrix_normalization  = self.getConfusionMaxtrix()
    return self.best_val_acc, self.best_val_loss, confusion_matrix, confusion_matrix_normalization
...
#100 - self.best_val_acc
  ...
  def getConfusionMaxtrix(self):
    ...
    with torch.no_grad():
        for idx in tqdm.tqdm(range(len(self.test_loader_ttau)), total=len(self.test_loader_ttau), leave=False):
            images, labels = self.test_loader_ttau[idx]
    ...
    dataframe_normalization = pd.DataFrame(cmn, index=class_names, columns=class_names)
    return dataframe, dataframe_normalization

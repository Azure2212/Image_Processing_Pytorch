...
class CbamBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_duck:
            #self.sigmoid = nn.Sigmoid()
            # self.wides = MidscopeConv2DBlock(channels, channels)
            # self.mids = WidescopeConv2DBlock(channels, channels)
            # self.sep = SeparatedConv2DBlock(channels, channels)
            self.wides = MidscopeConv2DBlock_upgrate(channels, channels)
            self.mids = WidescopeConv2DBlock_upgrate(channels, channels)
            self.sep = SeparatedConv2DBlock_upgrate(channels, channels)
    
    def forward(self, x):
        #print()
        #print(f"input shape Cbam : {x.shape}")
        duck_block = None
        if self.use_duck:
            x_wide = self.wides(x)
            x_mids = self.mids(x)
            x_sep = self.sep(x)
            duck_block = x_mids + x_wide + x_sep
            #print("MidScopeConv2D, WideScopeConv2D, SeparatedConv2D added to Cbam Block")
        x = self.ch_gate(x)
        x = self.sp_gate(x)
        if duck_block != None:
            #print("MidScopeConv2D, WideScopeConv2D, SeparatedConv2D added to Cbam Block")
            x = x + duck_block
            #x = self.sigmoid(x)
        #print(f"output shape Cbam : {x.shape}")
        return x
...
class CbamResUnit(nn.Module):
    ...
    def __init__(self,
        ...
        self.use_duck = use_duck
        #print(f"self.resize_identity = {self.resize_identity}")
        if use_duck == True:
            self.cbam = CbamBlock(channels=out_channels, use_duck = True)
        self.activ = nn.ReLU(inplace=True)
        ...
        for i, channels_per_stage in enumerate(channels):
            ...
            for j, out_channels in enumerate(channels_per_stage):
                ...
                stage.add_module("unit{}".format(j + 1), CbamResUnit(
                    ...
                    use_duck = True #if (j == (len(channels_per_stage)-1)) else False
                    #if ( (j == len(channels_per_stage)-1) or j == 0) else False
                    )) 
...
def cbam_resnet50_duck(**kwargs) -> nn.Module:
    """
...
def _test():
    ...
    for model in models:
        ...
        assert (model != cbam_resnet50_duck or weight_count == 28089624)
